...
export class ProductService {
  ...
  getProducts() {
    return this.fireStore.collection('/ProductTemp').snapshotChanges().pipe(
      map(products => {
        return products.map(product => {
          const data = product.payload.doc.data() as Product;
          const id = product.payload.doc.id;
          return this.fireStore.collection('/ProductTemp').doc(id).collection('reviews').get().pipe(
            map(reviews => {
  ...
  async deleteProduct(product: Product) {
    const productDocRef = this.fireStore.collection('/ProductTemp').doc(product.id).ref;
    const reviewsCollectionRef = this.fireStore.collection('/ProductTemp').doc(product.id).collection('reviews').ref;
